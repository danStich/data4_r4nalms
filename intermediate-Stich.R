...
## -----------------------------------------------------------------------------
ggplot(minnesummary, aes(x = Secchi, fill="tomato4")) +
  geom_histogram(bins = 15, color = "black") +
  xlab("Secchi depth (m)") +
  ylab("Frequency") +
  theme(
    axis.title.x = element_text(vjust = -1, hjust = 0.55),
    axis.title.y = element_text(vjust = 3)
  )

ggplot(minnesummary, 
       aes(x = GNIS.Class, y = Secchi, color = GNIS.Class, fill=GNIS.Class)) +
  geom_boxplot(alpha = 0.10, width = 0.33) +
  xlab("GNIS Class") +
  ylab("Secchi depth (m)") +
  theme_bw() +
  theme(
